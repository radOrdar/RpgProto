...
namespace FlyingWormConsole3.LiteNetLib.Utils
{
    ...
    public sealed class NetSerializer
    {
        ...
        private sealed class StructInfo
        {
            ...
            {
                Reference = obj;
                for (var i = 0; i < MembersCount; i++) WriteDelegate[i](writer);
            }

            public void Read(NetDataReader reader)
            {
                for (var i = 0; i < MembersCount; i++) ReadDelegate[i](reader);
            }
        }
                    ...
                    if (_registeredCustomTypes.TryGetValue(propertyType, out registeredCustomType))
                    {
                        if (array) //Array type serialize/deserialize
                        {
                            info.ReadDelegate[i] = reader =>
                            {
                                ...
                                for (var k = 0; k < arrLength; k++) arr.SetValue(registeredCustomType.ReadDelegate(reader), k);
                            ...
                            info.WriteDelegate[i] = writer =>
                            {
                                ...
                                for (var k = 0; k < arr.Length; k++) registeredCustomType.WriteDelegate(writer, arr.GetValue(k));
                            };
                        } else //Simple
                        {
                            info.ReadDelegate[i] = reader =>
