...
namespace FlyingWormConsole3.LiteNetLib
{
    internal sealed class NetSocket
    {
        ...
        private void ReceiveLogic(object state)
        {
            ...
            while (_running)
            {
                ...
                try
                {
                    ...
                    if (!bufferNetEndPoint.EndPoint.Equals(bufferEndPoint)) bufferNetEndPoint = new NetEndPoint((IPEndPoint)bufferEndPoint);
        ...
        public bool SendBroadcast(byte[] data, int offset, int size, int port)
        {
            try
            {
                var result = _udpSocketv4.SendTo(data, offset, size, SocketFlags.None, new IPEndPoint(IPAddress.Broadcast, port));
                if (result <= 0)
                    return false;
                if (IPv6Support)
                {
