    ...
    public sealed class NetManager
    {
        ...
        internal bool SendRaw(byte[] message, int start, int length, NetEndPoint remoteEndPoint)
        {
            ...
            if (errorCode != 0 && errorCode != 10040 && errorCode != 10065)
            {
                //Send error
                NetPeer fromPeer;
                if (_peers.TryGetValue(remoteEndPoint, out fromPeer)) DisconnectPeer(fromPeer, DisconnectReason.SocketSendError, errorCode, false, null, 0, 0);
        ...
        public void SendToAll(byte[] data, int start, int length, SendOptions options)
        {
            lock (_peers)
            {
                for (var i = 0; i < _peers.Count; i++) _peers[i].Send(data, start, length, options);
            }
        }
        ...
        public void Flush()
        {
            lock (_peers)
            {
                for (var i = 0; i < _peers.Count; i++) _peers[i].Flush();
            }
        }
        ...
        public void GetPeersNonAlloc(List<NetPeer> peers)
        {
            ...
            lock (_peers)
            {
                for (var i = 0; i < _peers.Count; i++) peers.Add(_peers[i]);
            }
        }
        ...
        {
            if (peer != null && _peers.ContainsAddress(peer.EndPoint)) DisconnectPeer(peer, DisconnectReason.DisconnectPeerCalled, 0, true, data, start, count);
        }
    }
}
#endif